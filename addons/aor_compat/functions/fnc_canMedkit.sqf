 ...
 *
 * Example:
 * [player, cursorTarget, "Head", "FieldDressing"] call unsci_aor_compat_fnc_canMedkit
 * Public: No
 */

params ["_medic", "_patient", "_bodyPart", "_bandage"];
TRACE_1("fnc_canMedkit",_this)
...
if (EGVAR(treatment,medkitFullHeals)) then {
    ...
    } forEach [(GET_OPEN_WOUNDS(_patient) getOrDefault [_bodyPart, []]), (GET_BANDAGED_WOUNDS(_patient) getOrDefault [_bodyPart, []]), (GET_STITCHED_WOUNDS(_patient) getOrDefault [_bodyPart, []])];
} else {
    {
        _x params ["_woundClassID", "_amountOf", "_bleeding"];
        ...
        if ((_className isNotEqualTo "AOR_BlamiteWound" && _className isNotEqualTo "AOR_SpikeWound") && (_amountOf * _bleeding > 0)) exitWith {
            _canBandage = true;
        };
