 ...
 * Prevents showing if an impalement is present - Im Kreepy
 *
 * Arguments:
 * 0: Medic <OBJECT>
 * 1: Patient <OBJECT>
 ...
 *
 * Example:
 * [player, cursorTarget, "Head", "FieldDressing"] call unsci_treatment_fnc_canMedigel
 *
 * Public: No
 */

params ["_medic", "_patient", "_bodyPart", "_bandage"];
TRACE_1("fnc_canMedigel",_this);
...

{
    ...
    if (!(_className == "UNSCI_BlamiteWound" || _className == "UNSCI_SpikeWound") && (_amountOf * _bleeding > 0)) exitWith {
        _canBandage = true;
    };
