...
/*
 * Authors: Im Kreepy
 * Handles the progress of removing Blamite.
 *
* Arguments:
 * 0: Arguments <ARRAY>
 *   0: Medic <OBJECT>
 ...
 *
 * Example:
 * [[objNull, player], 5, 10] call unsci_treatment_fnc_removingBlamiteProgress
 *
 * Public: No
 */
...
TRACE_1("fnc_removingBlamite",_this);
...
private _impalementWounds = [];
{
    ...
    if (_className isEqualTo "UNSCI_BlamiteWound") then {
...
if (_totalTime - _elapsedTime > ([_medic, _patient, _bodyPart] call FUNC(getBlamiteTreatmentTime)) - GVAR(treatmentTime_Imapalement)) exitWith {true};

// Stitch the first possible wound on the body part
private _plucked = [_patient, _bodyPart] call FUNC(removeBlamite);
