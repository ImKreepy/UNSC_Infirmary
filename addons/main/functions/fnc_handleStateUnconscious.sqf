...
if (ACEGVAR(medical,spontaneousWakeUpChance) > 0) then {
    if (_unit call ACEFUNC(medical_status,hasStableVitals)) then {
        private _lastWakeUpCheck = _unit getVariable ACEQGVAR(medical,lastWakeUpCheck);
        ...
        if (isNil "_lastWakeUpCheck") exitWith {
            ...
            _unit setVariable [ACEQGVAR(medical,lastWakeUpCheck), CBA_missionTime];
        };

        private _wakeUpCheckInterval = SPONTANEOUS_WAKE_UP_INTERVAL;
        if (ACEGVAR(medical,spontaneousWakeUpEpinephrineBoost) > 1) then {
            private _epiEffectiveness = ([_unit, "Epinephrine", false] call ACEFUNC(medical_status,getMedicationCount)) select 1;
            private _stimEffectiveness = ([_unit, "UNSCI_Stimulant", false] call ACEFUNC(medical_status,getMedicationCount)) select 1;
            _epiEffectiveness = _epiEffectiveness + _stimEffectiveness;
        ...
        if (CBA_missionTime - _lastWakeUpCheck > _wakeUpCheckInterval) then {
            ...
            if (random 1 <= ACEGVAR(medical,spontaneousWakeUpChance)) then {
                ...
                [ACEQGVAR(medical,WakeUp), _unit] call CBA_fnc_localEvent;
            };
        };
    } else {
        // Unstable vitals, procrastinate the next wakeup check
        private _lastWakeUpCheck = _unit getVariable [ACEQGVAR(medical,lastWakeUpCheck), 0];
        _unit setVariable [ACEQGVAR(medical,lastWakeUpCheck), _lastWakeUpCheck max CBA_missionTime];
    };
};
