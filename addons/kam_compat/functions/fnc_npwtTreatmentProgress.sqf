 ...
 *
 * Example:
 * [player, cursorObject, "LeftLeg"] call unsci_kam_compat_fnc_npwtTreatmentProgress;
 *
 * Public: No
 */
...
if (_totalTime - _elapsedTime > ([_patient, _patient, _bodyPart] call FUNC(getNPWTTreatmentTime)) - KAMGVAR(surgery,npwtTime)) exitWith {true};

if (_isBleeding) then {
    [ACEQGVAR(medical_treatment,bandageLocal), [_patient, _bodyPart, "Dressing"], _patient] call CBA_fnc_targetEvent; //TODO replace this
};
...
private _bodyPartDamage = _patient getVariable [ACEQGVAR(medical,bodyPartDamage), []];
...
_patient setVariable [ACEQGVAR(medical,bodyPartDamage), _bodyPartDamage, true];
